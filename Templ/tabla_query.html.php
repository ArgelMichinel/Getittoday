            ...
            <tbody  id="data_table">
                ...
                for ($i=0 ; $i < $rows; $i++) {
                    ...
                    foreach ($packets[$i] as $key => $value) {
                        ...
                        if (($key != 'id_num') && ($key != 'status') && ($key != 'sender_id') && ($key != 'city' ) && ($key != 'delivery_preference') && ($key != 'street_name') && ($key != 'street_number') && ($key != 'receiver_phone') && ($key != 'cadete') && ($key != 'sticker') && ($key != 'id_ship')) {echo ('    <td class="' . $key . '">'. $value.'</td>'. PHP_EOL);}
                        if ($key === 'id_ship' )  {
                            echo ('    <td class="' . $key . '">'. $value.' <i class="fa fa-qrcode" aria-hidden="true"></i></td>'. PHP_EOL);
                        }
                        ...
                        if ($key === 'sticker' )  {
                            if (is_null($value)) {
                                echo ('    <td style="display: none;">'. '(vacio)'.'</td>'. PHP_EOL);
                            } else {
                                echo ('    <td style="display: none;">'. $value.'</td>'. PHP_EOL);
                            }
                        }
                        ...
                        if ($key === 'city' )  {
                            if ($packets[$i]['delivery_preference'] == 1 )  {
                                echo ('    <td class="city" style="background-color: magenta; color: white;"">'. $value.'</td>'. PHP_EOL);
                            } else {
                                echo ('    <td class="city">'. $value.'</td>'. PHP_EOL);
                            }
                        }
